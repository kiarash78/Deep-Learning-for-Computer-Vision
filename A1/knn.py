...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i, x1 in enumerate(x_train):
    for j, x2 in enumerate(x_test):
      dists[i, j] = (lambda x: torch.sum(x**2))(x1 - x2)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i, x1 in enumerate(x_train):
    a = x_test - x1
    b = torch.sum(a**2, axis=[i for i in range(a.dim()) if i > 0])
    dists[i,:] = b
...
def compute_distances_no_loops(x_train, x_test):
  ...
  x2 = (x_train**2).sum(axis=1).view(num_train, 1)
  y2 = (x_test**2).sum(axis=1)
  xy = torch.mm(x_train, x_test.t())
  dists = -2 * xy + x2 + y2
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  y_pred = torch.mode(y_train[
    torch.topk(-dists, k=k, dim=0)[1]
  ], dim=0)[0]
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for fold in range(num_folds):
      train_indexes = [i for i in range(num_folds) if i != fold]
      x_train = torch.cat([x_train_folds[i] for i in train_indexes])
      y_train = torch.cat([y_train_folds[i] for i in train_indexes])
      classifier = KnnClassifier(x_train, y_train)
      x_test = x_train_folds[fold]
...
def knn_get_best_k(k_to_accuracies):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  best_k = max(k_to_accuracies.keys(),
    key=lambda k: torch.mean(torch.tensor(k_to_accuracies[k])))
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  return best_k
