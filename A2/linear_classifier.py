...
def svm_loss_vectorized(W, X, y, reg):
  ...
  # Replace "pass" statement with your code
  scores = X.mm(W)
  normalized_scores = scores - scores[torch.arange(num_train), y].view(-1, 1) + 1
  loss_is_positive = normalized_scores > 0
  loss_is_positive[torch.arange(num_train), y] = False
  normalized_scores[~loss_is_positive] = 0
  loss = torch.sum(normalized_scores)
  loss /= num_train
  loss += reg * torch.sum(W * W)
  #############################################################################
  #                             END OF YOUR CODE                              #
  ...
  #############################################################################
  # Replace "pass" statement with your code
  dW += X.t().mm(loss_is_positive.double())
  to_substract = torch.zeros_like(loss_is_positive).long()
  to_substract[torch.arange(num_train), y] = loss_is_positive.sum(axis=1)
  dW -= X.t().mm(to_substract.double())
  dW /= num_train
  dW += 2 * reg * W
  #############################################################################
  #                             END OF YOUR CODE                              #
...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = 10 ** (torch.linspace(-2, +3, 5))
  regularization_strengths = 10 ** (torch.linspace(-5, -2, 5))
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  y_val_hat = cls.predict(X_val)
  train_acc = (y_train_hat == y_train).float().mean()
  val_acc = (y_val_hat == y_val).float().mean()
...
def softmax_loss_naive(W, X, y, reg):
  ...
  # Replace "pass" statement with your code
  y_pred = X.mm(W)
  y_pred -= y_pred.max(dim=1)[0].view(-1, 1)
  N, D = X.shape
  for i in range(N):
    c = y[i]
    loss += -torch.log(torch.exp(y_pred[i, c])/torch.exp(y_pred[i]).sum())
    dW[:,c] -= X[i]
    dW += torch.outer(X[i], torch.exp(y_pred[i])/torch.exp(y_pred[i]).sum())
  loss /= N
  loss += torch.sum(W * W) * reg
  dW /= N
  dW += 2 * W * reg
  #############################################################################
  #                          END OF YOUR CODE                                 #
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  y_pred_exp = y_pred_exp / y_pred_exp.sum(axis=1).view(-1, 1)
  loss = -torch.log(y_pred_exp)[torch.arange(N), y].sum()
  loss /= N
  loss += reg * torch.sum(W * W)

  dW += X.t().mm(y_pred_exp)
  dW -= X.t().mm(torch.eye(C).cuda().double()[y])
  dW /= N
  dW += 2 * W * reg
  #############################################################################
  #                          END OF YOUR CODE                                 #
...
def softmax_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = 10 ** (torch.linspace(-2, -1, 4))
  regularization_strengths = 10 ** (torch.linspace(-5, -4, 2))
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
