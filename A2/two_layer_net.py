...
def nn_forward_pass(params, X):
    ...
    hidden = X.mm(W1) + b1
    scores = torch.maximum(hidden, torch.tensor(0).cuda()).mm(W2) + b2
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    scores -= scores.max(dim=1)[0].view(-1, 1)
    scores_exp = torch.exp(scores)
    scores_exp = scores_exp / scores_exp.sum(axis=1).view(-1, 1)
    loss = -torch.log(scores_exp)[torch.arange(N), y].sum()
    loss /= N
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    running_grad = scores_exp
    running_grad -= torch.eye(C).cuda().double()[y]
    running_grad /= N
    grads['b2'] = running_grad.sum(axis=0)
    grads['W2'] = torch.maximum(h1, torch.zeros(1, device='cuda')).t().mm(running_grad)
    grads['W2'] += 2 * W2 * reg
    running_grad = running_grad.mm(W2.t())
    running_grad[h1 <= 0] = 0
    grads['b1'] = running_grad.sum(axis=0)
    grads['W1'] = X.t().mm(running_grad)
    grads['W1'] += 2 * W1 * reg
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_train(params, loss_func, pred_func, X, y, X_val, y_val,
  ...
  for it in range(num_iters):
    ...
    for parameter in params:
      params[parameter] -= learning_rate * grads[parameter]
    #########################################################################
    #                             END OF YOUR CODE                          #
    #########################################################################
...
def nn_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = 10 ** torch.linspace(-2, 0, 2)
  hidden_sizes = 2 ** torch.arange(8, 10)
  regularization_strengths = [0, 0.001, 0.01, 0.1]
...
def find_best_net(data_dict, get_param_set_fn):
  ...
  output_size = torch.cat((y_train, y_val)).max() + 1
  def grid_search(li):
    ptrs = [0] * len(li)
    while True:
      yield tuple(li[i][ptr] for i, ptr in enumerate(ptrs))
      big_ptr = 0
      while big_ptr < len(li):
        ptrs[big_ptr] += 1
        if ptrs[big_ptr] == len(li[big_ptr]):
          ptrs[big_ptr] = 0
          big_ptr += 1
        else:
          break
      if big_ptr == len(li):
        break
  params_set = get_param_set_fn() 
  nets = {}
  val_accs = {}
  stats = {}
  for params in grid_search(params_set):
    lr, h_size, reg, decay = params
    lr = lr.item()
    net = TwoLayerNet(input_size, h_size, output_size)
    stat = net.train(X_train, y_train, X_val, y_val, lr, decay, reg,
      num_iters=2000, batch_size=1000)
    y_val_pred = net.predict(X_val)
    acc = (y_val == y_val_pred).float().mean()
    val_accs[params] = acc
    nets[params] = net
    stats[params] = stat
  best_params = max(val_accs, key=lambda params: val_accs[params])
  best_net = nets[best_params]
  best_stat = stats[best_params]
  best_val_acc = val_accs[best_params]
  print(best_params)
  #############################################################################
  #                               END OF YOUR CODE                            #
  #############################################################################
